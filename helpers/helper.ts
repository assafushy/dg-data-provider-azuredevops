...
export class Helper {
  ...
  public static suitList:Array<suiteData> = new Array<suiteData>();
    public static findSuitesRecursive(planId:string,url:string, project:string,suits:any,foundId:string,recursive:boolean):Array<suiteData>
    {
        ...
        for (let i= 0;i < suits.length;i++)
        {
            ...
            if (suits[i].parentSuiteId != 0)
              ...
              let suit:suiteData = new suiteData(suits[i].title,suits[i].id,foundId,this.level++);
                  suit.url = url+project+"/_testManagement?planId="+planId+"&suiteId="+suits[i].id+"&_a=tests";
    ...
    public static  LevelBuilder(results: Query,foundId:string):Array<Workitem>{
      for (let i= 0;i < results.workItems.length;i++)
      {
        ...
        {    
          ...
          if (results.workItems[i].Source.toString() == foundId)
          { 
