...
describe("Common functions module - tests", () => {
  test("should return all collecttion projects", async () => {
    ...
    let json = await restApi.GetProjects();
    expect(json.count).toBeGreaterThan(1);
  });
  test("should return project by name", async () => {
    let restApi = new AzureRestApi(orgUrl, token);
    let json = await restApi.GetProjectByName("doc-gen-test");
    expect(json.name).toBe("doc-gen-test");
  });
  test("should return project by id", async () => {
    let restApi = new AzureRestApi(orgUrl, token);
    let json = await restApi.GetProjectByID(
      "799d6cb6-e18c-4b61-8f5a-845993aeac96"
    );
    expect(json.name).toBe("doc-gen-test");
  });
  test("should return all collecttion link types", async () => {
    let restApi = new AzureRestApi(orgUrl, token);
    let json = await restApi.GetCllectionLinkTypes();
    expect(json.count).toBeGreaterThan(1);
  });
}); //describe
describe("Work items manipulations - tests", () => {
  test("should create a new work item", async () => {
    ...
    let res = await restApi.CreateNewWorkItem(
      "doc-gen-test",
      body,
      "Epic",
      true
